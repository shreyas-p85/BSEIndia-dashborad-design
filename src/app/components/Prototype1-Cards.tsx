...
import { ChevronRight, BarChart3, FileText, TrendingUp, Database, Users, Download, Building2, UserCircle, ArrowLeft, Activity, Search, Menu } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <header className="bg-white shadow-sm border-b">
        <img src={image_header_img_5} alt="BSE Header" className="w-full" />
        ...
        <img src={image_Sensex_data_4} alt="Market Data" className="w-full" />
      </header>
      ...
      <main className="flex-1 container mx-auto px-4 md:px-6 py-8">
        {currentView !== 'main' && (
          <button
            ...
            className="flex items-center gap-2 bg-white text-[#243166] hover:bg-gray-50 px-4 md:px-5 py-2 md:py-3 rounded-xl shadow-md hover:shadow-lg mb-8 transition-all border border-gray-200 font-medium text-sm md:text-base"
          >
              ...
              {Object.entries(dashboardData).map(([key, category]) => {
                ...
                return (
                  <button
                    ...
                    className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-200 hover:scale-105 hover:border-[#243166]"
                  >
                    <div className={`${category.color} p-6 md:p-8 text-white relative overflow-hidden min-h-[160px] md:min-h-[180px] flex flex-col justify-center`}>
                      <div className="absolute top-0 right-0 w-24 h-24 md:w-32 md:h-32 bg-white opacity-10 rounded-full -mr-12 md:-mr-16 -mt-12 md:-mt-16"></div>
                      <Icon className="w-10 h-10 md:w-12 md:h-12 mb-3 relative z-10" />
                      <h3 className="text-sm md:text-base font-bold leading-tight relative z-10">{category.title}</h3>
                      {category.subtitle && (
                        <p className="text-xs mt-1 opacity-90 relative z-10">{category.subtitle}</p>
                      )}
                    </div>
                    <div className="p-3 md:p-4 bg-gradient-to-b from-white to-gray-50">
                      <div className="flex items-center justify-between text-gray-700">
                        <span className="text-xs md:text-sm font-medium">{category.subCategories.length} {category.subCategories.length === 1 ? 'category' : 'categories'}</span>
                        <ChevronRight className="w-4 h-4 md:w-5 md:h-5 group-hover:translate-x-2 transition-transform text-[#243166]" />
                      </div>
                    </div>
                  </button>
                );
              ...
              <>
                <div className="mb-8 md:mb-10">
                  <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                    <div className={`${dashboardData[selectedCategory as keyof typeof dashboardData].color} p-3 md:p-4 rounded-2xl shadow-lg`}>
                      {(() => {
                    ...
                    <div>
                      ...
                      <p className="text-sm md:text-base text-gray-600 mt-1">Browse categories and access detailed information</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
                  {dashboardData[selectedCategory as keyof typeof dashboardData].subCategories.map((subCat) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`${dashboardData[selectedCategory as keyof typeof dashboardData].color} bg-opacity-10 p-6 border-b-4 ${dashboardData[selectedCategory as keyof typeof dashboardData].color.replace('bg-', 'border-')}`}>
                          <div className="flex items-center gap-3">
                            <div className={`${dashboardData[selectedCategory as keyof typeof dashboardData].color} p-3 rounded-xl shadow-md`}>
                        ...
                        <div className="p-6 bg-white">
                          ...
                          <div className="mt-4 pt-4 border-t border-gray-100">
                            <p className="text-xs text-gray-500 text-center">{subCat.items.length} items available</p>
                          </div>
                        </div>
                      </div>
                    );
      ...
      <footer className="mt-8 md:mt-16">
        <img src={image_footer_img_3} alt="BSE Footer" className="w-full" />
      </footer>
    </div>
  );
}
